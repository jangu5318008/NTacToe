...
void TextGraphics::setConsoleSize(int cols, int rows) {
    // \x1b[8;ROWS;COLS t
...
void TextGraphics::putCharAt(int i, int j, char ch, Color color)
{
    pixels[i][j] = ch;
    colors[i][j] = color;
}
...
void TextGraphics::putStringAt(int i, int j, const std::string& text, Color color)
{
    for (size_t n = 0; n < text.size() && j + n < WIDTH; n++)
    {
        pixels[i][j + n] = text.at(n);
        colors[i][j + n] = color;
    }
}
